// A tiny, dependency-free syntax highlighter shared by the docs page and the
// playground editor. Tokens are built as DOM nodes — no innerHTML — and
// unknown text passes through as-is.
...
export function highlightOCaml(source) {
...
export function highlightReason(source) {
  const out = makeEmitter(document.createDocumentFragment());
  const n = source.length;
  let i = 0;
  let tagContext = false; // the identifier right after "<" or "</" is a tag
...
export function highlightDune(source) {
  const out = makeEmitter(document.createDocumentFragment());
  const n = source.length;
  let i = 0;
