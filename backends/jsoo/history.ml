(* Browser history / location FFI via the js_of_ocaml FFI.
   ...
   One of the two implementations of the [History] virtual module declared by
   src/history.mli; the other is backends/melange/history.ml. It exposes the
   same value signature, so src/router.ml — the very same compiled module the
   Melange build uses, not a copy — runs against it unchanged. The routing port
   is, like the DOM port, just this one file plus [Dom]. Bindings go through
   [Js.Unsafe], mirroring the Melange [external ... [@@mel.*]] declarations
   one-for-one. *)
